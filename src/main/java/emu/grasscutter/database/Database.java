...
/** Complicated manager of the MongoDB database. Handles caching, data operations, and more. */
public interface Database {
    ...
    static void saveAll(List<? extends DatabaseObject<?>> objects) {
        ...
        var gameObjects = objects.stream().filter(DatabaseObject::isGameObject).toList();
        var accountObjects = objects.stream().filter(o -> !o.isGameObject()).toList();
    ...
    /** Starts the auto-save thread. Runs every 10s 1000为1秒. */
